...
function selectView(view) {
  const button=qsa('[data-view]').find(item=>item.dataset.view===view);
  if (!button) return;
  qsa('[data-view]').forEach(item=>item.classList.toggle('active',item.dataset.view===view));
  qsa('.view-panel').forEach(panel=>panel.classList.toggle('hidden',panel.dataset.panel!==view));
  const workspace=qs('.breadcrumb strong').textContent;
  qs('#page-title').textContent=view==='overview'?workspace:view==='bom'?'BOM Explorer':view==='scenarios'?'Scenario lab':'Supplier watch';
}

qsa('[data-view]').forEach(button=>button.addEventListener('click',()=>selectView(button.dataset.view)));
qsa('.workspace-item').forEach(button=>button.addEventListener('click',()=>{
  qsa('.workspace-item').forEach(item=>item.classList.toggle('selected',item===button));
  const workspace=button.dataset.workspace||button.textContent.replace('›','').trim();
  qs('.breadcrumb strong').textContent=workspace;
  if (qs('#mobile-workspace')) qs('#mobile-workspace').value=workspace;
  selectView('overview');
  toast(`Switched to ${workspace}.`);
}));
qs('.new-workspace').addEventListener('click',()=>toast('New workspace setup is ready for the next project.'));
if (qs('#mobile-workspace')) qs('#mobile-workspace').addEventListener('change',(event)=>{
  const workspace=event.target.value;
  qsa('.workspace-item').forEach(item=>item.classList.toggle('selected',item.dataset.workspace===workspace));
  qs('.breadcrumb strong').textContent=workspace;
  selectView('overview');
  toast(`Switched to ${workspace}.`);
});
...
qs('#view-risk-btn').addEventListener('click',()=>{selectView('bom');qsa('.filter-button').find(item=>item.dataset.filter==='risk').click();});
...
qs('#create-scenario-btn').addEventListener('click',()=>toast('Scenario draft created. Add location, escalation, and supplier assumptions to continue.'));
